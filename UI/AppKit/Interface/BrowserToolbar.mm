...
- (instancetype)initWithWindow:(GNUstepBrowserWindow*)window
{
    self = [super init];
    if (self) {
        NSLog(@"BrowserToolbar: init starting");
        fflush(stderr);
        ...
        NSLog(@"BrowserToolbar: init complete");
        fflush(stderr);
    }
    return self;
}
...
{
    NSLog(@"BrowserToolbar: locationFieldAction");
    fflush(stderr);
...
- (NSToolbarItem*)createTabOverviewItem
{
    if (!_tab_overview_toolbar_item) {
        ...
        auto* button = [[NSButton alloc] initWithFrame:NSMakeRect(0, 0, 24, 24)];
        [button setTitle:@"\u25A6"];  // Unicode square with horizontal fill (grid-like)
        [button setBordered:YES];
        [button setTarget:self];
...
{
    NSLog(@"BrowserToolbar: itemForItemIdentifier: %@", identifier);
    fflush(stderr);
